...
def get_apm_services(url, shard, auth):
    log.debug("Calling apm services API with auth={auth} shard={shard}".format(auth=auth, shard=shard))
    headers = {'Cookie':"dogweb={auth}; DD-PSHARD={shard}".format(auth=auth, shard=shard)}
    params = dict(env='production')
    r = web.get('https://' + url + '/api/v1/trace/services', params=params, headers=headers)
...
def main(wf):
    try:
        ...
        def apm_wrapper():
            return get_apm_services(api_url, shard, auth)
